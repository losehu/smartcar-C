...
const int8_t CenterCalAry[16] = //dy:0-15//+3+2
        { 27, 30, 34, 36, 43, 50, 55, 62, 69, 76, 83, 90, 97, 105, 112, 119 };
...
//两点之间补线
uint8 TwoPoint_AddingLine(uint8 x1, uint8 y1, uint8 x2, uint8 y2, int16 *p,
        uint8 StopYsite) {
    int16 i = 0;
...
void ScanBorderStateReset(void) {
    ...
    Base.last_first_left_find_to_lose_y=Base.first_left_find_to_lose_y;
    Base.last_first_right_find_to_lose_y=Base.first_right_find_to_lose_y;
    Base.last_first_left_find_to_lose_x=Base.first_left_find_to_lose_x;
      Base.last_first_right_find_to_lose_x=Base.first_right_find_to_lose_x;
    Base.first_left_find_to_lose_y = 0;
    Base.first_right_find_to_lose_y = 0;
}
...
//                && Img.LeftBorderFindFlag[i + 1] == 'F'
//                && Img.LeftBorderFindFlag[i + 2] == 'T'
//                && Img.LeftBorderFindFlag[i + 3] == 'T') {
//            Base.first_left_find_to_lose_y = i + 2;
//            Base.first_left_find_to_lose_x = Img.LeftBorder[i + 2];
//
//            break;
//        }
//    }
...
//                && Img.RightBorderFindFlag[i + 1] == 'F'
//                && Img.RightBorderFindFlag[i + 2] == 'T'
//                && Img.RightBorderFindFlag[i + 3] == 'T') {
//            Base.first_right_find_to_lose_y = i + 2;
//            Base.first_right_find_to_lose_x =Img.RightBorder[i + 2];
//
//            break;
//        }
//
//    }
    if (Base.first_left_find_to_lose_y == 0 && Base.first_right_find_to_lose_y == 0
        ...
        Base.first_left_find_to_lose_x = Img.RightBorder[ROW - 1]
               -RoadWide0/2;
        Base.first_right_find_to_lose_x = Img.RightBorder[ROW - 1]
                +RoadWide0/2;
...
uint8 StrightJudge(void) {
    ...
    if (Base.Topline >= 40) {
        ...
        return 0;
    //判断
    if (Base.Topline >= 55 && Straight.LeftCnt40up > 12
            && Straight.LeftCnt > 15)
        Straight.LeftLong = 'T';
    else
        Straight.LeftLong = 'F';
    if (Base.Topline >= 55 && Straight.RightCnt40up > 12
            && Straight.RightCnt > 15)
    ...
    if (Base.Topline >= 40 && Straight.RightCnt > 25
            && Straight.RightLong == 'F')
...
uint8 ForkCheckPeakPiont(void) {
    ...
    if (Base.firstroaderror <= RoadWide0 && Base.LoseRoadLine >= ROW - 10) {
        //路宽不正常且有丢线计数
        for (Ysite = Base.firstroaderror;
                Ysite < Base.firstroaderror + RoadWide0; Ysite++) {
    ...
    if (ForkRoadErrorCnt >= RoadWide0 * 2 / 3) {  //从黑色部分中线向两边扫线，扫到白色截止，作为三叉的内边界
        for (Ysite = Base.firstroaderror + RoadWide0 * 4 / 3;
                Ysite >= Base.firstroaderror; Ysite--) {
            if (ImageUsed[ROW - 1 - Ysite][ForkScanStartCol] == BlackPoint) {
                //向左扫线
                for (Xsite = ForkScanStartCol; Xsite >= LOutBorderCol;
                        Xsite--) {
                ...
                //向右扫线
                for (Xsite = ForkScanStartCol; Xsite <= ROutBorderCol;
                        Xsite++) {
                    if (ImageUsed[ROW - 1 - Ysite][Xsite] == WhitePoint
                            || Xsite == COL - 1) {
                ...
                //更新扫线起始列
                ForkScanStartCol = (Fork.ForkLeftBorder[Ysite]
                        + Fork.ForkRightBorder[Ysite]) / 2;
                if ((Fork.ForkRightBorder[Ysite] - Fork.ForkLeftBorder[Ysite])
                        <= 2) {
...
uint8 ForkCheckTurnOk(void) {
    ...
    for (Ysite = 0; Ysite < RoadWide0; Ysite++) {
        ...
        if ((ForkTurnOkLSlopeNormalCnt >= RoadWide0 / 2
                && ForkTurnOkRSlopeNormalCnt >= RoadWide0 / 2)
    ...
    if (Fork.state == ForkInTurn || Fork.state == ForkOutTurn) {
        if (Fork.In_Direct == 'R') {
            TwoPoint_AddingLine(Fork.L_Inflexion.x, Fork.L_Inflexion.y,
                    Fork.Peak.x, Fork.Peak.y,
                    &Img.LeftBorder_Repair[Fork.L_Inflexion.y], 0);
            for (Ysite = Fork.L_Inflexion.y; Ysite < Fork.Peak.y; Ysite++) {
                Img.Center_Repair[Ysite] = (Img.LeftBorder_Repair[Ysite]
                        + Img.RightBorder_Repair[Ysite]) / 2;
            }
            for (Ysite = Fork.Peak.y; Ysite < ROW - 1; Ysite++) {
                Img.Center_Repair[Ysite] = Img.RightBorder_Repair[Ysite]
                        - RoadWide0 / 2;
                Img.Center_Repair[Ysite] = limit_ab(Img.Center_Repair[Ysite], 0,
                        COL - 1);
            }
        } else if (Fork.In_Direct == 'L') {
            TwoPoint_AddingLine(Fork.R_Inflexion.x, Fork.R_Inflexion.y,
                    Fork.Peak.x, Fork.Peak.y,
                    &Img.RightBorder_Repair[Fork.R_Inflexion.y], 0);
            for (Ysite = Fork.R_Inflexion.y; Ysite < Fork.Peak.y; Ysite++) {
                Img.Center_Repair[Ysite] = (Img.RightBorder_Repair[Ysite]
                        + Img.LeftBorder_Repair[Ysite]) / 2;
            }
            for (Ysite = Fork.Peak.y; Ysite < ROW - 1; Ysite++) {
                Img.Center_Repair[Ysite] = Img.LeftBorder_Repair[Ysite]
                        + RoadWide0 / 2;
...
//    uint8 ValidRow = 0;
    for (Ysite = 1; Ysite < RoadWide0 + 10; Ysite++) {
        if ((ImageUsed[ROW - 1 - Ysite][CenterCol] == BlackPoint
                && ImageUsed[ROW - Ysite][CenterCol] == WhitePoint)
                || (ImageUsed[ROW - 1 - Ysite][CenterCol] == WhitePoint
                        && ImageUsed[ROW - Ysite][CenterCol] == BlackPoint))
            ColorChangeCnt++;
        if (ColorChangeCnt >= 8)
            break;
    }
...
//    uint8 ValidRow = 0;
    for (Xsite = LBorderCol; Xsite < RBorderCol; Xsite++) {
        if ((ImageUsed[ROW - 1 - RoadWide0 / 2][Xsite] == BlackPoint
                && ImageUsed[ROW - 1 - RoadWide0 / 2][Xsite + 1] == WhitePoint)
                || (ImageUsed[ROW - 1 - RoadWide0 / 2][Xsite] == WhitePoint
                        && ImageUsed[ROW - 1 - RoadWide0 / 2][Xsite + 1]
                                == BlackPoint))
            ColorChangeCnt++;
    ...
    for (Xsite = LBorderCol; Xsite < RBorderCol; Xsite++) {
        if ((ImageUsed[ROW - 1 - RoadWide0 / 2 + 3][Xsite] == BlackPoint
                && ImageUsed[ROW - 1 - RoadWide0 / 2 + 3][Xsite + 1]
                        == WhitePoint)
                || (ImageUsed[ROW - 1 - RoadWide0 / 2 + 3][Xsite] == WhitePoint
                        && ImageUsed[ROW - 1 - RoadWide0 / 2 + 3][Xsite + 1]
                                == BlackPoint))
            ColorChangeCnt++;
        if (ColorChangeCnt >= 14)
...
uint8 BarnOutCheckBorderDistance(void) {
    ...
    for (Ysite = 15; Ysite < 45; Ysite++) {
        ...
        if (BarnOutBorderYsiteL != 255 && BarnOutBorderYsiteR != 255) {
            BarnOutBorderYsite = (BarnOutBorderYsiteL + BarnOutBorderYsiteR)
                    / 2;
            break;
        }
    }
...
uint8 BarnInCheckBorderDistance(void) {
    ...
    if (Base.firstroadnormal <= 15) {
        ...
        for (Ysite = Base.firstroadnormal; Ysite < Base.firstroadnormal + 10;
                Ysite++) {
            if (Img.RoadNormalFlag[Ysite] == 'T')
                BarnInAfterBarnNormalCnt++;
            if (BarnInAfterBarnNormalCnt >= 5) {
...
uint8 BarnOutCheckInflexionPiont(void) {
    ...
    if (Base.firstroadnormal < 20) {
        for (Ysite = Base.firstroadnormal;
                Ysite < min_ab(Base.firstroadnormal + 20, ROW - 5); Ysite++) { //搜最近的20行
            if (myabs(Img.LeftSlope[Ysite])
                    >= 0.67|| Img.LeftSlope[Ysite] == NoneSlope) {
    ...
    if (Base.firstroadnormal < 20) {
        for (Ysite = Base.firstroadnormal;
                Ysite < min_ab(Base.firstroadnormal + 20, ROW - 5); Ysite++) {
            if (myabs(Img.RightSlope[Ysite])
                    >= 0.67|| Img.RightSlope[Ysite] == NoneSlope) {
    ...
    Barn.L_Inflexion.x = limit_ab(Barn.L_Inflexion.x, LBorderCol - 5,
            LBorderCol + 5);
    Barn.L_Inflexion.y = limit_ab(Barn.L_Inflexion.y, 0, 10);
    Barn.R_Inflexion.x = limit_ab(Barn.R_Inflexion.x, RBorderCol - 5,
            RBorderCol + 5);
...
uint8 BarnInCheckInflexionPiont(void) {
    ...
    //左转进车库
        if (Barn.OutDir == 'L') {
            //左下拐点//即车库右拐点
            for (Ysite = Base.firstroadnormal;
                    Ysite < min_ab(Base.firstroadnormal + 15, ROW - 5);
                    Ysite++) {
                if (myabs(Img.LeftSlope[Ysite])
                        >= 0.67|| Img.LeftSlope[Ysite] == NoneSlope) {
                    Barn.R_Inflexion.y = max_ab(Base.firstroadnormal,
                            Ysite - 1);
            ...
            //左上拐点//即车库左拐点
            for (Ysite = Barn.R_Inflexion.y + (RoadWide0 * 2) / 3;
                    Ysite < Barn.R_Inflexion.y + (RoadWide0 * 4) / 3; Ysite++) {
                if (Img.RoadNormalFlag[Ysite] == 'T') {
            ...
            //左上拐点向上路宽正常计数
            for (Ysite = Barn.L_Inflexion.y; Ysite < Barn.L_Inflexion.y + 15;
                    Ysite++) {
                if (Img.RoadNormalFlag[Ysite] == 'T')
                    Up_NormalCnt++;
                if (Up_NormalCnt >= 10)
        ...
        //右转进车库
            ...
            for (Ysite = Base.firstroadnormal;
                ...
                if (myabs(Img.LeftSlope[Ysite])
                        >= 0.67|| Img.LeftSlope[Ysite] == NoneSlope) {
                    Barn.L_Inflexion.y = max_ab(Base.firstroadnormal,
                            Ysite - 1);
            ...
            //右上拐点向上路宽正常计数
            for (Ysite = Barn.R_Inflexion.y; Ysite < Barn.R_Inflexion.y + 15;
                    Ysite++) {
                if (Img.RoadNormalFlag[Ysite] == 'T')
                    Up_NormalCnt++;
                if (Up_NormalCnt >= 10)
...
uint8 BarnInCheckOk(void) {
    ...
    if (Base.Topline <= RoadWide0 / 3
            && Base.firstleftlose.y <= RoadWide0 * 2 / 3
            && Base.firstrightlose.y <= RoadWide0 * 2 / 3
        ...
        for (Ysite = BarnInRoadNormalCnt;
                Ysite < BarnInRoadNormalCnt + RoadWide0 / 2; Ysite++) {
            if (Img.RoadNormalFlag[Ysite] == 'F'
                    && Img.LeftBorderFindFlag[Ysite] == 'F'
                    && Img.RightBorderFindFlag[Ysite] == 'F')
...
{ 'L', 20, 20, 100, 230, 30 }, { 'R', 20, 15, 100, 230, 30 }, };
...
void BarnAddingLine(void) {
    //出车库补线右转出
    if (Barn.FindFlag == 'T' && Barn.OutDir == 'R'
            && Barn.state == BarnOutTurn) {
        ...
        for (Ysite = Barn.L_Inflexion.y; Ysite < Barn.R_Inflexion.y + RoadWide0;
                Ysite++) {
            Img.Center_Repair[Ysite] = Img.LeftBorder_Repair[Ysite]
                    + RoadWide0 * 2 / 3;
        }
    ...
    //出车库补线左转出
    if (Barn.FindFlag == 'T' && Barn.OutDir == 'L'
            && Barn.state == BarnOutTurn) {
        ...
        for (Ysite = Barn.R_Inflexion.y; Ysite < Barn.L_Inflexion.y + RoadWide0;
                Ysite++) {
            Img.Center_Repair[Ysite] = Img.RightBorder_Repair[Ysite]
                    - RoadWide0 * 2 / 3;
        }
...
//        }
        TwoPoint_AddingLine(LBorderCol, 0, RBorderCol, RoadWide0,
                &Img.LeftBorder_Repair[0], 0);
        for (Ysite = 0; Ysite < RoadWide0; Ysite++) {
            Img.Center_Repair[Ysite] = Img.Center_Repair[Ysite]
                    + RoadWide0 * 2 / 3;
        }
    }
}
...
{ 'R', 50, 155, 0, 42, 35, 250, 240, 45 }, { 'L', 100, 105, 0, 41, 7, 250, 250,
        15 },
...
void CircleJudge(void) {
    //电磁信号判断环岛
    if (ElcJudge() && Circle.FindFlag == 'F'
            && Wheel.Distance >= Circle.CircleEndDis) {
        uint8 InfoGroupx = 0;
        Circle.FindFlag = 'T';
        Circle.CircleNum++;
    ...
    //出环岛左转
    if (Circle.FindFlag == 'T' && Circle.state == CircleIn && Circle.dir == 'L'
            && Line5_35.LeftLose >= 20 && LineALL.RightLose >= 10
            && ICMIntegrate.TurnLeft_I > 250.0f) {
    ...
    if (Circle.FindFlag == 'T' && Circle.state == CircleOutStraight
            && Circle.dir == 'L' && ElcSum_14 <= Circle.CircleELC14
            && Line5_35.LeftNormal >= 15 && Line5_35.RightNormal >= 15) {
        ExSpeed = NormalSpeed;
        BeepTick(1, 100);
        Circle.state = CircleOut;
        Circle.FindFlag = 'F';
    ...
    //出环岛右转
    if (Circle.FindFlag == 'T' && Circle.state == CircleIn && Circle.dir == 'R'
            && Line5_35.RightLose >= 20 && LineALL.LeftLose >= 10
            && ICMIntegrate.TurnRight_I > 250.0f) {
        Circle.state = CircleOutTurn;
    ...
    //出环完成环岛标志置位
    if (Circle.FindFlag == 'T' && Circle.state == CircleOutStraight
            && Circle.dir == 'R' && ElcSum_14 <= Circle.CircleELC14
            && Line5_35.LeftNormal >= 15 && Line5_35.RightNormal >= 15) {
        ExSpeed = NormalSpeed;
        Circle.state = CircleOut;
        Circle.FindFlag = 'F';
...
void CircleAddingLine(void) {
    ...
    if (Circle.FindFlag == 'T'
        ...
        if (Circle.dir == 'L')    //右线补左线
            ...
            TwoPoint_AddingLine(0, 5, Circle.LeftUpInflexion.x,
                    Circle.LeftUpInflexion.y, &Img.LeftBorder_Repair[5], 55);
        ...
        if (Circle.dir == 'R') {
            ...
            TwoPoint_AddingLine(127, 5, Circle.RightUpInflexion.x,
                    Circle.RightUpInflexion.y, &Img.RightBorder_Repair[5], 55);
    ...
    if (Circle.FindFlag == 'T' && Circle.state == CircleInTurn) {
        if (Circle.dir == 'L') {
            ...
            if (Circle.LeftUpInflexion.y >= 5) {
                TwoPoint_AddingLine(127, 5, Circle.LeftUpInflexion.x,
                        Circle.LeftUpInflexion.y, &Img.RightBorder_Repair[5],
                        0);
        ...
        if (Circle.dir == 'R') {
            ...
            if (Circle.RightUpInflexion.y >= 5) {
                TwoPoint_AddingLine(0, 5, Circle.RightUpInflexion.x,
                        Circle.RightUpInflexion.y, &Img.LeftBorder_Repair[5],
                        0);
    ...
    if (Circle.FindFlag == 'T' && Circle.state == CircleOutTurn) {
        if (Circle.dir == 'L') {
            TwoPoint_AddingLine(127, 5, 10, Column10.firstblackY20up,
                    &Img.RightBorder_Repair[5], 0);
        ...
        if (Circle.dir == 'R') {
            TwoPoint_AddingLine(0, 5, 107, Column117.firstblackY20up,
                    &Img.LeftBorder_Repair[5], 0);
    ...
    if (Circle.FindFlag == 'T' && Circle.state == CircleOutStraight) {
        if (Circle.dir == 'L') {
            for (Ysite = 5; Ysite < Base.Topline; Ysite++) {
                Img.LeftBorder_Repair[Ysite] = Img.RightBorder_Repair[Ysite]
                        - RoadWide0;    //令向右偏移
                Img.Center_Repair[Ysite] = (Img.LeftBorder_Repair[Ysite]
                        + Img.RightBorder_Repair[Ysite]) / 2;
            }
        ...
        if (Circle.dir == 'R') {
            for (Ysite = Base.firstrightlose.y; Ysite < Base.Topline; Ysite++) {
                Img.RightBorder_Repair[Ysite] = Img.LeftBorder_Repair[Ysite]
                        + RoadWide0;
                Img.Center_Repair[Ysite] = (Img.LeftBorder_Repair[Ysite]
                        + Img.RightBorder_Repair[Ysite]) / 2;
            }
...
void RampJudge(void) {
    ...
    min_ab(Ramp.LeftSlopeSatisfy,Ramp.RightSlopeSatisfy) > 15
            ...
            || (Ramp.LeftSlopeSatisfy > 30 && LineALL.RightLose >= 25
                    && Base.LeftMax.y >= 55)
            || (Ramp.RightSlopeSatisfy > 30 && LineALL.LeftLose >= 25
                    && Base.RightMin.y >= 55)) && Ramp.FindFlag == 'F'
            && Ramp.RecoverFlag == 'F') {
        BeepTick(1, 100);
        Ramp.MayRampFlag = 'T';
        Ramp.State = UpRamp;
    ...
    else if (Ramp.MayRampFlag == 'T'
            && Wheel.Distance >= (Ramp.MayRampDis + Ramp.MayRampFlagClearDis)) {
        Ramp.MayRampFlag = 'F';
        Ramp.State = EndRamp;
        ExSpeed = NormalSpeed;
    }
    ...
    if (Ramp.RecoverFlag == 'T'
            && Wheel.Distance > (Ramp.EndRampDis + Ramp.RecoverDis)) {
        //BEEP_OFF;
        Ramp.State = EndRamp;
        Ramp.RecoverFlag = 'F';
        ExSpeed = NormalSpeed;
...
uint8 CrossCheckInflexionPoint(void) {
    ...
    //左下拐点向上搜线确认线数
    if (InRange(myabs(Cross.LD_Inflexion.x - Cross.RD_Inflexion.x),
            RoadWide0 - 10, RoadWide0 + 10)
            && InRange(Cross.LD_Inflexion.x, LOutBorderCol, CenterCol)
            && InRange(Cross.RD_Inflexion.x, CenterCol, ROutBorderCol))
        for (Ysite = Cross.LD_Inflexion.y;
                Ysite < (Cross.LD_Inflexion.y + RoadWide0); Ysite++) {
            if (Img.LeftBorderFindFlag[Ysite] == 'F')
    ...
    if (InRange(myabs(Cross.LD_Inflexion.x - Cross.RD_Inflexion.x),
            ...
            && InRange(Cross.LD_Inflexion.x, LOutBorderCol, CenterCol)
            && InRange(Cross.RD_Inflexion.x, CenterCol, ROutBorderCol))
        for (Ysite = Cross.RD_Inflexion.y;
                Ysite < (Cross.RD_Inflexion.y + RoadWide0); Ysite++) {
        ...
        return 0;
    //向上搜左上拐点
    for (Ysite = Cross.LD_Inflexion.y + RoadWide0 / 3;
            Ysite < Cross.LD_Inflexion.y + (RoadWide0 * 4) / 3; Ysite++) {
    ...
    //向上搜右上拐点
    for (Ysite = Cross.RD_Inflexion.y + RoadWide0 / 3;
            Ysite < Cross.RD_Inflexion.y + (RoadWide0 * 4) / 3; Ysite++) {
        if (myabs(Img.RoadWideError[Ysite]) <= 10) {
    ...
    //左上拐点向上搜线确认线数
    if (InRange(myabs(Cross.LU_Inflexion.x - Cross.RU_Inflexion.x),
            RoadWide0 - 10, RoadWide0 + 10)
            && InRange(Cross.LU_Inflexion.x, LOutBorderCol, CenterCol)
            && InRange(Cross.RU_Inflexion.x, CenterCol, ROutBorderCol))
        for (Ysite = Cross.LU_Inflexion.y; Ysite < Cross.LU_Inflexion.y + 15;
                Ysite++) {
            if (Img.LeftBorderFindFlag[Ysite] == 'T'
    ...
    if (InRange(myabs(Cross.LU_Inflexion.x - Cross.RU_Inflexion.x),
            ...
            && InRange(Cross.LU_Inflexion.x, LOutBorderCol, CenterCol)
            && InRange(Cross.RU_Inflexion.x, CenterCol, ROutBorderCol))
        for (Ysite = Cross.RU_Inflexion.y; Ysite < Cross.RU_Inflexion.y + 15;
                Ysite++) {
    ...
    //all
    if (Cross.LD_UpAbnormalCnt >= RoadWide0 / 3
            && Cross.RD_UpAbnormalCnt >= RoadWide0 / 3
...
void CrsosJudge(void) {
    ...
    if ((Cross.FindFlag == 'T'
            && Wheel.Distance > (Cross.CrossLength + Cross.CrossStartDis))) {
...
void CrossAddingLine(void) {
    if (CrossCheckInflexionPoint() == 1) {
        TwoPoint_AddingLine(Cross.LD_Inflexion.x, Cross.LD_Inflexion.y,
                ...
                &Img.LeftBorder_Repair[Cross.LD_Inflexion.y], 0);
        TwoPoint_AddingLine(Cross.RD_Inflexion.x, Cross.RD_Inflexion.y,
                Cross.RU_Inflexion.x, Cross.RU_Inflexion.y,
                &Img.RightBorder_Repair[Cross.RD_Inflexion.y], 0);
        for (Ysite = min_ab(Cross.LD_Inflexion.y, Cross.RD_Inflexion.y);\

                Ysite < max_ab(Cross.LU_Inflexion.y, Cross.RU_Inflexion.y);
                Ysite++) {
            Img.Center[Ysite] = (Img.LeftBorder_Repair[Ysite]
                    + Img.RightBorder_Repair[Ysite]) / 2;
        }
    }
}
