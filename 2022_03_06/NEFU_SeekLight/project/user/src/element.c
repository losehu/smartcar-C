...
void judge_door() {
    if (seed_right_x - seed_left_x < 5) {
    }else return;
    int cnt=0;
    for(int i=0;i<COL-1;i++)     if(ImageUsed[seed_left_y][i]!=0&&ImageUsed[seed_left_y][i+1]==0)cnt++;

    if(cnt>=5)         stop_flag = 1;
...
void judge_circle_left() {
    if (circle_flag.find_left_circle == 0) {
        ...
        {
            if(Img.RightBorder[i]-Img.LeftBorder[i]>Img.RightBorder[i-1]-Img.LeftBorder[i-1])sum_temp++;
        }
        if(sum_temp<18)return;
