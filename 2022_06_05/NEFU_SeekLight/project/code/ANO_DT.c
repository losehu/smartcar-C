...
{
    Waveform[USERDATA_1]   = (int16)(ExSpeed_L-Wheel_L.SpeedNow);
    Waveform[USERDATA_2]   = (int16)(ExSpeed_R-Wheel_R.SpeedNow);
...
int error_cnt=0;
unsigned char img_bin[6+send_ROW*send_COL/8];
#define sended_image *PerImg_ip
//#define sended_image SimBinImage
...
for (int i = 0; i < send_ROW; i++) {
    for (int j = 0; j < send_COL; j++) {
        if ((sended_image[i][j]) == 0)  img_bin[cnt_sum] = img_bin[cnt_sum] << 1;
...
img_bin[send_COL*send_ROW/8+3]='9',img_bin[send_COL*send_ROW/8+4]='9',img_bin[send_COL*send_ROW/8+5]='9';
uart_write_buffer(WRIELESS_UART_INDEX, img_bin, send_COL*send_ROW/8+6);   // 发送最后的数据
}
