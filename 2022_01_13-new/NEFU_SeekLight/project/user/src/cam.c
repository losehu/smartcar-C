...
uint8 GetBorder(void) {
    ...
    //获取首行起始列
    Ysite = StartScanRow;
    for (Xsite = CenterCol - RoadWide0 / 2 - 5;
            Xsite < CenterCol + RoadWide0 / 2 + 5; Xsite++) {
    ...
    if (Base.StartSearchColumn_1st <= 0)
        Base.StartSearchColumn_1st = CenterCol;    //使车子过急转弯
    ...
    for (Ysite = StartScanRow; Ysite < ROW; Ysite++) {
        ...
        for (Xsite = Base.StartSearchColumn; Xsite >= 0; Xsite--) {
            if (ImageUsed[ROW - 1 - Ysite][Xsite] == BlackPoint
                    && !(Xsite == 0
                            || ImageUsed[ROW - 1 - Ysite][Xsite] == GrayPoint)) //设置赛道左起始点
                    {
                ...
                break;
                    ...
                    == 0|| ImageUsed[ROW - 1 - Ysite][Xsite] == GrayPoint) {
            ...
            if (Ysite >= 3 && Img.LeftBorderFindFlag[Ysite] == 'T'
                ...
                Img.LeftSlope[Ysite] = ((float) (Img.LeftBorder[Ysite])
                        - (float) (Img.LeftBorder[Ysite - 3])) / 3;    //计算斜率
        ...
        for (Xsite = Base.StartSearchColumn; Xsite < COL; Xsite++) {
            if (ImageUsed[ROW - 1 - Ysite][Xsite] == BlackPoint
                    && !(Xsite == (COL - 1)
                            || ImageUsed[ROW - 1 - Ysite][Xsite] == GrayPoint)) //扫到黑线且不接近边缘
                    {
                ...
                break;
                    ...
                    == (COL - 1)|| ImageUsed[ROW - 1 - Ysite][Xsite] == GrayPoint) {
                Img.RightBorder[Ysite] = COL - 1;
                Img.RightBorderFindFlag[Ysite] = 'F';
                if (Base.firstrightlose.y == 0
                        && Base.firstrightnormal.y != 0) {
            ...
            if (Ysite >= 3 && Img.RightBorderFindFlag[Ysite] == 'T'
                ...
                Img.RightSlope[Ysite] = ((float) (Img.RightBorder[Ysite - 3])
                        - (float) (Img.RightBorder[Ysite])) / 3;    //计算斜率
        ...
        //更新扫线起始点
        Base.StartSearchColumn =
                (Img.LeftBorder[Ysite] + Img.RightBorder[Ysite]) / 2;
        //更新赛道宽度多个标志
...
//            Img.RoadNormalFlag[Ysite + 2] == 'F' && 。
        ...
        //更新顶边
        //有效行截止 需要实地测试
        if (Img.RoadWide[Ysite] < RoadWide0/*Img.RoadWide[Ysite] > 10 +RoadWide0*/
        && Base.ToplineFindFlag == 'F') {
        ...
        if (Ysite < StartScanRow + 1)
            ...
            if (Img.RightBorderFindFlag[Ysite] == 'T'
                ...
                error_right_flag = 0, error_left_flag = 0;
            }
                ...
                Img.Center[Ysite] = CenterCol;
                    ...
                    && Img.LeftBorderFindFlag[Ysite] == 'T') //丢了右边线
                            {
                error_left_flag = 0;
                if (Img.RightBorderFindFlag[Ysite - 1] == 'T'
                        && !error_right_flag) {
                    error_right_flag = 1; //10    -50
                    error_right_tmp = Img.LeftBorder[Ysite] + RoadWide0 / 2
                            - Img.Center[Ysite - 1];
                    Img.Center[Ysite] = Img.Center[Ysite - 1] + 1;
                } else if (Img.RightBorderFindFlag[Ysite - 1] == 'F'
                        && error_right_flag) {
                    ...
                    && Img.LeftBorderFindFlag[Ysite] == 'F')  //丢了左边
                            {
                error_right_flag = 0;
                if (Img.LeftBorderFindFlag[Ysite - 1] == 'T'
                        && !error_left_flag) {
                    error_left_flag = 1; //10    -50
                    error_left_tmp = Img.RightBorder[Ysite] - RoadWide0 / 2
                            - Img.Center[Ysite - 1];
                    Img.Center[Ysite] = Img.Center[Ysite - 1] - 1;
                } else if (Img.LeftBorderFindFlag[Ysite - 1] == 'F'
                        && error_left_flag) {
...
uint8 Fork_Find_Peak(void) {
    ...
    for (Ysite = min_ab(Column64.firstblackY + 10, 55); Ysite >= 5; Ysite--) {
        ...
        xPoint0 = (Fork.ForkLeftBorder[Ysite] + Fork.ForkRightBorder[Ysite])
                / 2;
...
uint8 ForkJudge(void) {
    //入三岔
    if (Fork.FindFlag == 'F' && Fork_Find_Peak() != 0 && LineALL.LeftLose >= 25
            && LineALL.RightLose >= 25 && Fork_Avoid_Cross() == 1
            && Ret_BlackCenterLine_Length() >= 3 && InRange(ElcSum_14, 20, 80)
            &&
            myabs(Column10.Line30_60_Black - Column117.Line30_60_Black) < 12
            && Column64.firstblackY
                    < max_ab(Column10.firstblackY, Column117.firstblackY) &&
    ...
    if (Fork.FindFlag == 'T' && Fork.state == ForkInTurn
            && (ICMIntegrate.TurnRight_I > 20.0f
                    || ICMIntegrate.TurnLeft_I > 20.0f)) {
        Fork.state = ForkIn;
    ...
    //出三岔
    if (Base.Topline <= 55 && Fork.FindFlag == 'T' && LineALL.AllLose >= 20
            && Fork.state == ForkIn && ElcSum_14 <= 30)    //赛道到不了顶边
                    {
    ...
    if (Fork.FindFlag == 'T' && Fork.state == ForkOutTurn
            && (ICMIntegrate.TurnRight_I > 20.000f
                    || ICMIntegrate.TurnLeft_I > 20.000f)) {
        Fork.state = ForkOut;
        Fork.FindFlag = 'F';
        ICMIntegrate.Yaw_I_Enable = 'F';
        ExSpeed = NormalSpeed;
...
void ForkAddingLine(void) {
    ...
    if (Fork.state == ForkInTurn) {
        if (Fork.In_Direct == 'R') {
            Fork_Find_Peak();
            TwoPoint_AddingLine(0, 5, Fork.Peak.x, Fork.Peak.y,
            ...
            Fork_Find_Peak();
            TwoPoint_AddingLine(127, 5, Fork.Peak.x, Fork.Peak.y,
...
uint8 Line_Ret_WhiteBlackChange_VaildN(void) {
    ...
    uint8 ValidRow = 0;
    for (Ysite = Barn.StartLineFoundY; Ysite < (Barn.StartLineFoundY + 30);
            Ysite++) {
        Y0_ColorChange[Ysite] = 0;
        for (Xsite = (64 - RoadWide0 / 2); Xsite < (Ysite + RoadWide0 / 2);
                Xsite++) {
            if ((ImageUsed[59 - Ysite][Xsite] == 0
                    && ImageUsed[59 - Ysite][Xsite - 1] == 255)
                    || (ImageUsed[59 - Ysite][Xsite] == 0
                            && ImageUsed[59 - Ysite][Xsite + 1] == 255))
...
void BarnJudge(void) {
    ...
    if (Barn.state == BarnOutStraight
            && ((Wheel.Distance >= (Barn.BarnStartDistance + 0.200f)
                    && Barn.OutDir == 'R')
                    || (Wheel.Distance >= (Barn.BarnStartDistance + 0.100f)
                            && Barn.OutDir == 'L'))) {
        CarInfo.ClosedLoopMode = LR_SingleControl;
        if (Barn.OutDir == 'L') {
            ExSpeed_L = 20;
    ...
    if (Barn.FindFlag == 'F' && Barn.state == BarnOutOK
        ...
        if (Column64.Line30_60_White > 20 && Row56.LeftToRightFirstWhite < 64
                && Row56.RightToLeftFirstBlack > 64
    ...
    //方案2停车
    //允许转向
    if (Barn.FindFlag == 'T' && Barn.state == BarnTurnStraight
            && BarnTurnInOk() <= 5) {
        //BEEP_ON;
    ...
    if (Barn.FindFlag == 'T' && Barn.state == BarnInStraight
            && Wheel.Distance
                    >= (Barn.BarnInDistanceCount + Barn.BarnInRunDis)) {
        Barn.state = BarnIn;
        Barn.FindFlag = 'T';
        CarInfo.CAR_PROTECT_MODE = 'T';
...
void BarnAddingLine(void) {
    //出车库补线 右上端点
    if (Barn.FindFlag == 'T' && Barn.state == BarnOutTurn
            && Barn.OutDir == 'R') {
        TwoPoint_AddingLine(0, 5, Base.firstrightnormal.x,
                Base.firstrightnormal.y, &Img.LeftBorder_Repair[5], 0);
    ...
    if (Barn.FindFlag == 'T' && Barn.state == BarnOutTurn
            && Barn.OutDir == 'L') {
        TwoPoint_AddingLine(127, 5, Base.firstleftnormal.x,
                Base.firstleftnormal.y, &Img.RightBorder_Repair[5], 0);
    ...
    if (Barn.FindFlag == 'T' && Barn.state == BarnInTurn
            && Barn.OutDir == 'L') {
        TwoPoint_AddingLine(127, 5, 0, Column10.firstblackY20up,
                &Img.RightBorder_Repair[5], 0);
    ...
    if (Barn.FindFlag == 'T' && Barn.state == BarnInTurn
            && Barn.OutDir == 'R') {
        TwoPoint_AddingLine(0, 5, 127, Column117.firstblackY20up,
                &Img.LeftBorder_Repair[5], 0);
        for (Ysite = 6; Ysite < Base.firstrightnormal.y; Ysite++) {
            Img.Center_Repair[Ysite] = Img.LeftBorder_Repair[Ysite]
                    + RoadWide0 / 2;
...
uint8 CrossCheckInflexionPoint(void) {
    ...
    for (Ysite = Cross.LD_Inflexion.y;
        ...
        if (Img.LeftBorderFindFlag[Ysite]
                == 'F'/*Img.LeftSlope[Ysite] >= 0.3f && Img.LeftSlope[Ysite] < 3.0f*/)
    ...
    for (Ysite = Cross.RD_Inflexion.y;
        ...
        if (Img.RightBorderFindFlag[Ysite]
                == 'F'/*Img.RightSlope[Ysite] >= 0.3f && Img.RightSlope[Ysite] < 3.0f*/)
    ...
    for (Ysite = Cross.LU_Inflexion.y; Ysite < Cross.LU_Inflexion.y + 15;
            Ysite++) {
        if (Img.LeftBorderFindFlag[Ysite] == 'T'
                && myabs(Img.RoadWideError[Ysite]) <= 10)
    ...
    RoadWide0 - 10, RoadWide0 + 10)
            && InRange(myabs(Cross.LU_Inflexion.x - Cross.RU_Inflexion.x),
            RoadWide0 - 10, RoadWide0 + 10)
            ...
            && Cross.LD_UpAbnormalCnt >= RoadWide0 / 3
...
void dfs(int start_x,int start_y)
{
    int i;
        for( i=start_x;i>=0;i--)
        {
              if(ImageUsed[start_y][i]==BlackPoint) break;
