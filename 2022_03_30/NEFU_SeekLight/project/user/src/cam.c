...
void ScanBorderStateReset(void)
{
    //-------------------Base---------------------//
    //-------------------Base---------------------//
      Base.firstleftnormal.y = ROW - 1;
      Base.firstleftnormal.x = 0;
      Base.firstrightnormal.y = ROW - 1;
      Base.firstrightnormal.x = 0;
      Base.firstleftlose.y = ROW - 1;
      Base.firstleftlose.x = 0;
      Base.firstrightlose.y = ROW - 1;
    ...
    uint8 GetBorder(void)
    {
        ...
        for(Ysite = StartScanRow;Ysite < ROW;Ysite ++)
        {
            ...
            if (Img.LeftBorder[Ysite] < 0 + lose_error || Img.LeftBorder[Ysite] > COL - 1 - lose_error || \
                ImageUsed[ROW - Ysite - 1][max_ab(Img.LeftBorder[Ysite] - 1, 0)] == GrayPoint)
            ...
            if (Img.RightBorder[Ysite] < 0 + lose_error || Img.RightBorder[Ysite] > COL - 1 - lose_error || \
                ImageUsed[ROW - Ysite - 1][min_ab(Img.RightBorder[Ysite] + 1, COL - 1)] == GrayPoint)
            ...
            if(Ysite == ROW - 1 && Base.ToplineFindFlag == 'F'){
                Base.ToplineTemp = (max_py1+max_py)/2;
            }
    ...
    //                Base.LoseRoadLine == ROW - 1)
            ...
            if (Ysite < StartScanRow + 1) {
                Img.Center[Ysite] = COL / 2;//底部赋中值
            }
            else if (Ysite > Base.LoseRoadLine&&Base.LoseRoadLine - 5>=0)
                Img.Center[Ysite] = Img.Center[Base.LoseRoadLine - 5];
            else {
                //此处应对有丢边界的中线进行处理
                //边界直接平移
