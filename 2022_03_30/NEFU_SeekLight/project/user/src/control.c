...
int16 fork_speed=190;
uint16_t Ackerman_kl = 195;    //左右差速系数
uint16_t Ackerman_kr = 185;
int16 ExSpeed_L = 0;
int16 ExSpeed_R = 0;
 ...
 *@Sample       :AllImageDeal();
 **/
uint8 CenterCal1(void)
{
    static int32 point_center0 = 0;
    static int32 point_center1 = 0;
    static int32 point_center2 = 0;
    //计算当前行的偏移量
    float excursion[60];
    float sumcenter = 0;
#define CenterCalLine 27
#define CenterCalNum 37
    for(Ysite = CenterCalLine;Ysite < CenterCalLine + CenterCalNum;Ysite ++){
        excursion[Ysite] = (float)(Img.Center_Repair[Ysite] - (COL/2)) / (float)(RoadWide0 / 2) * 100;
    }
    for(Ysite = CenterCalLine;Ysite < CenterCalLine + CenterCalNum ;Ysite ++){
        sumcenter += excursion[Ysite];
    }
    point_center0 = (int32)(sumcenter / (CenterCalNum) * 2.0f);
