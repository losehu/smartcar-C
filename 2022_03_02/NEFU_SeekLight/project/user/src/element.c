     ...
     if(temp_flag==0){
         if (max_py <= ROW * 0.9||max_py1<=ROW*0.9)
               ...
               temp_flag=1;
                  ...
                  if (max_py >= ROW * 0.8||max_py1>=ROW*0.8)  return;
int sum_temp=0;
         for(int i=0;i<ROW;i++)sum_temp+=Img.RightBorder[i];
         if(sum_temp<(COL-1-10)*ROW)return;
         int useful=0;
for(int i=0;i<25;i++)
{
 if(Img.RightBorderFindFlag[i]=='F'&&Img.LeftBorderFindFlag[i]=='T')useful++;
}
if(useful<20)return;
                             int        block_cnt = 0;
                                     for (int i = 0; i < 5; i++) {
