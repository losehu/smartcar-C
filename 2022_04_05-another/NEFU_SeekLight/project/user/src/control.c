...
uint16_t Ackerman_kl = 170;    //左右差速系数170
uint16_t Ackerman_kr = 170;//170
int16 ExSpeed_L = 0;
int16 ExSpeed_R = 0;
...
//    MotorWrite();
    ...
    EncoderRead();
    if((abs( Wheel_L.SpeedNow)<=80||abs( Wheel_R.SpeedNow)<=80)&&!stop_flag)
    { low_speed_cnt++;
    if(low_speed_cnt>=400)
    {
        stop_flag=1;
           ExSpeed_SUM=0;
           ips114_clear(RED);
