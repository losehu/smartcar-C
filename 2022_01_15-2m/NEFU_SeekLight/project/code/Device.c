...
//    gpio_init(LED1_PIN, GPO, GPIO_LOW, GPIO_PIN_CONFIG);//E9是左编码器方向引脚

//    gpio_init(LEDR_PIN, GPO, GPIO_LOW, GPIO_PIN_CONFIG);
//    gpio_init(LEDG_PIN, GPO, GPIO_LOW, GPIO_PIN_CONFIG);
//    gpio_init(LEDB_PIN, GPO, GPIO_LOW, GPIO_PIN_CONFIG);
...
void Beep_IntCnt(void)
{
    ...
    {
        Beep.Cnt ++;
        if(Beep.Cnt %Beep.Time == 0) gpio_toggle_level(BEEP_PIN);
