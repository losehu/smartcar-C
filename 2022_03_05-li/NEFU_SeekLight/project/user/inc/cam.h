...
//    uint8 MaybeCorssFlag;
    float ForkTurnLenth;//三叉转进转出的长度
    float ForkLenth;//三叉最小长度，防止进入时重复判断三叉
    //记录
    uint8 ForkNum;//当前经过的三叉数
    float FindDis;//发现三叉时的距离
    float InDis;//进入三叉后的距离
    //按键可调
    uint8 ForkAmount;//一圈的三叉总数
    uint8 Dir;
//    float TurnInK;
//    float TurnOutK;
    uint16 TurnSpeed;
...
//extern uint8 ForkRoadErrorCnt;
//extern uint8 ForkBorderNormalCnt;
