...
void ShowRepairLine(void)
{
    ...
    {
        if(Ysite==stack_top||Ysite==stack_top1)continue;
...
uint8 ForkShow(void)
{
    ...
    while(KeyLast != onepress)
    {
        ...
        ips114_show_int16(120,1,Fork.TurnSpeed);
        ips114_show_int16(120,2,Fork.TurnSpeed);
        //调整参数
        if(KeyNext == onepress)
        {
            KeyNext = nopress;
            switch(ForkChooseItem)
            {
                case(0):ParamsAdjustShort((short *)&Fork.ForkAmount,120,0,1);
                    break;
                case(1):ParamsAdjustShort((short *)&Fork.TurnSpeed,120,0,1);
                    break;
                case(2):ParamsAdjustShort((short *)&Fork.TurnSpeed,120,0,1);
                    break;
