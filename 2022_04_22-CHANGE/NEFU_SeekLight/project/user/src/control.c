...
int16 ExSpeed = 400; //320 NormalSpeed
int16 ExSpeed_SUM = 400; //250
int16 ExSpeed_SUM_tmp=400;
int16 circle_speed = 400 ; //240
int16 fork_speed=275;
uint16_t Ackerman_kl = 140;    //左右差速系数170
uint16_t Ackerman_kr = 140;//120
int16 ExSpeed_L = 0;
int16 ExSpeed_R = 0;
...
//         Ackerman_kr=0.2*last_Ackerman_kr+0.8*((1.0*now_speed/   ExSpeed)*85);//95 90
         ...
         int a=1.0*abs(point_center*0.7+point_center0*0.2+point_center1*0.1)/400.0f*50;
         int b=1.0*(100-max(max_py,max_py1))/99.0f*50;
         int c=ServoAdd/800.0f*50;
         ExSpeed=ExSpeed-max(max(a,b),c);
...
//    }
    if (CarInfo.ClosedLoopMode != NoLoop) {
        ...
        if(Wheel.SpeedNow<(SpeedOut_L+SpeedOut_R/2)-15)
        {
            pwm_l = 999;
               pwm_r = 999;
        }else if(Wheel.SpeedNow>(SpeedOut_L+SpeedOut_R/2)+15)
        {
            pwm_l = -999;
                          pwm_r = -999;
        }
