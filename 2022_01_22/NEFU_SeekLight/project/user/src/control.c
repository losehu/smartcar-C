...
void AllImageDeal(void) {
  ...
  //  system_cnt(1);
    ...
    //uint8_t aa[100][114];
   // for(int i=0;i<100;i++)for(int j=0;j<114;j++)aa[ROW-1-i][j]=ImageUsed[i][j];

   // camera_send_image(DEBUG_UART_INDEX, &aa[0][0], 114*100);
 ...
 //   system_cnt(1);//开始计时
   ...
   judge_circle();
    //cal_middle_line();
  // ips114_show_int32(1, 1, system_cnt(0), 8);//显示时间
}
...
void DiffSpeedControl(void) {
    static uint16_t Ackerman_kl = 260;    //左右差速系数
    static uint16_t Ackerman_kr = 260;
