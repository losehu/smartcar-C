...
void judge_circle_right() {
    ...
    if (circle_flag.find_right_circle == 0) {
        ...
        if( LNor[L2]<=ECL_circle||LNor[L3]<=ECL_circle)return;
if(max_py<ROW*0.9||max_py1<ROW*0.9)return;
if(left_lost_cnt>=right_lost_cnt-RoadWide0/2)return;
ips114_clear(WHITE);
...
//        CenterCalMaxRow = 10;
        ...
        for (int i = ROW - 1; i >= ROW - 10; i--) {
            for (int j = search_col-10; j < search_col ; j++) {
                if (ImageUsed[i][j] == 0)
                    block_cnt++;
            }
        }
...
ips114_clear(BROWN);
        ...
        cntt++;
        kp1_tmp = ServoPIDParam[0][0];
        ExSpeed_SUM = ExSpeed_SUM_tmp-60;

        ServoPIDParam[0][0] = 2.3;

    } else if (circle_flag.enter_left_circle == 0) {
int flag=0;
        if(!(Base.first_left_find_to_lose_y> Base.last_first_left_find_to_lose_y+10))
    flag++;
        int cntt=0;
        for(int i=0;i<30;i++)if(Img.RightBorderFindFlag[i]=='T') cntt++;
        if(cntt<=28)return;

         //   if(Img.RightBorder[1]-Img.LeftBorder[1]>COL*0.7&&flag)return;
        if(flag)return;
   // return;

//    Base.first_left_find_to_lose_y=Base.last_first_left_find_to_lose_y;
//    Base.first_left_find_to_lose_x=Base.last_first_left_find_to_lose_x;
...
//circle_flag.enter_left_circle =1;
        ...
        int cntt=0;
             for(int i=0;i<30;i++)if(Img.RightBorderFindFlag[i]=='T') cntt++;
             if(cntt<=28)return;
        if (connect.room != 4&&connect.room != 2)
            return;

        if(Img.RightBorder[1]-Img.LeftBorder[1]>COL*0.7)return;
...
//        if (!(max_py <= ROW * 0.6 || max_py1 <=  ROW * 0.6))
//            return;
        ...
        for(int i=0;i<5;i++)
        {
            for(int j=ROW-1-5-cal;j<ROW-cal;j++)
            {
                if(ImageUsed[i][j]!=0) cntt++;
            }
        }
        if(cntt<=20)return;
      ...
      if(  stc_flag.out_circle==0)return;
      init_flag();
         CenterCalMinRow = CenterCalMinRow_tmp;
         CenterCalMaxRow = CenterCalMaxRow_tmp;
         ExSpeed_SUM = ExSpeed_SUM_tmp;
         ips114_clear(BLACK);
         ServoPIDParam[0][0] = kp1_tmp;//        if (temp_flag == 0) {
         out_cnt=3000;
         return;
