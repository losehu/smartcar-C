...
cross cross_flag;
 stc stc_flag;
 float diff_speed_tmp; //3700diff_speed
...
//    stc_sec=gpio_get_level(STC_SEC_FLAG);
    ...
    if(LNow[0]<=500&&LNow[3]<=500||LNow[1]<=500&&LNow[2]<=500)
    {
        ExSpeed_SUM = 0;
        ExSpeed = 0;
               ips114_clear(GREEN);
    ...
    }else if(ELC_Sum_14>5200&&ELC_Sum_23>5200&&ELC_Sum_1234>=11000)
    {
...
STC_read();
  if (circle_flag.judge_left == 1 || circle_flag.judge_right==1)      judge_circle();
    ...
    if (ramp_flag.judge == 1)     judge_ramp();
...
void judge_circle_right() {
    if (circle_flag.find_right_circle == 0) {
      ...
      ips114_clear(WHITE);
        gpio_high(BEEP_PIN);
        if(left_lost_cnt>RoadWide0*0.5)return;
        int cnt_die=0;
        for(int i=1;i<ROW-1;i++)  {
            if(ImageUsed[ROW-1-i][ROW-1-5]!=0&&i>40)break;

            if(Img.RightBorder[i]<Img.RightBorder[i-1])cnt_die++;
            if(cnt_die==3)return;
        }
        close_judge(0);
        circle_flag.find_right_circle = 1;
...
int cnt_die=0;
for(int i=1;i<ROW-1;i++){
    if(ImageUsed[ROW-1-i][5]!=0&&i>40)break;
    if(Img.LeftBorder[i]>Img.LeftBorder[i-1])cnt_die++;
    if(cnt_die==3)return;
}
ips114_clear(BROWN);
        gpio_high(BEEP_PIN);
...
void judge_ramp()
{
    ...
    int ramp_cnt=0;
         for(int i=0;i<ROW;i++)
             ...
             if(Img.RightBorder[i]-Img.LeftBorder[i]>RoadWide0*1.1&&Img.LeftBorder[i]>=5&&Img.RightBorder[i]<=COL-1-5)ramp_cnt++;

         if(ramp_cnt<ROW*0.8)return;
         ramp_cnt=0;
         for(int i=3;i<ROW;i++)

             if(Img.RightBorder[i]-Img.LeftBorder[i]>Img.RightBorder[i-3]-Img.LeftBorder[i-3])ramp_cnt++;

         if(ramp_cnt<ROW*0.5)return;
ExSpeed_SUM=280;
ips114_clear(MAGENTA);
ramp_flag.find=1;
gpio_high(BEEP_PIN);
close_judge(5);
out_cnt=1000;
    }
    else if(ramp_flag.out==0){
        if(out_cnt>10)return;
        int ramp_cnt=0;
                for(int i=0;i<ROW;i++)
       ...
       ramp_flag.out=1;
        ...
        if(max_ab(max_py,max_py1)<ROW*0.9)return;
        int ramp_cnt=0;
        for(int i=0;i<ROW/5;i++)
              if(Img.RightBorder[i]-Img.LeftBorder[i]<RoadWide0*1.2&&Img.RightBorder[i]-Img.LeftBorder[i]>RoadWide0*0.8)ramp_cnt++;
        if(ramp_cnt<ROW/8)return;
        gpio_low(BEEP_PIN);
        init_flag();
    }
}
