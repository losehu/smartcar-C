...
//布尔数值范围判断
bool InRange(int32 x, float _min, float _max) {
    if (x <= _max && x >= _min)
        return 1;
    else
        return 0;
...
for(int Ysite=0;Ysite<ROW;Ysite++){
    ...
    if(Ysite >= 3 && Img.RightBorderFindFlag[Ysite] == 'T' &&
        ...
        Img.RightSlope[Ysite] = ((float)(Img.RightBorder[Ysite]) - (float)(Img.RightBorder[Ysite - 3]))/3.0f;
    ...
    //  左斜率
      if(Ysite >= 3 && Img.LeftBorderFindFlag[Ysite] == 'T' &&
          ...
          Img.LeftSlope[Ysite] = ((float)(Img.LeftBorder[Ysite]) - (float)(Img.LeftBorder[Ysite - 3]))/3.0f;
