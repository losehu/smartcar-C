...
static  uint8_t             bluetooth_ch9141_buffer[BLUETOOTH_CH9141_BUFFER_SIZE];                                // 对应第一个 无线串口 数据存放数组

static           uint8_t    bluetooth_ch9141_data;
static  volatile uint8_t    bluetooth_ch9141_at_reply[30];      // 接收at命令的缓存
static  volatile uint8_t    bluetooth_ch9141_at_reply_index;    // 接收at命令的缓存
static  volatile uint32_t   bluetooth_ch9141_at_cmd_state;      // AT 指令状态 <0-不在 AT 模式> <1-AT 模式等待响应> <2-AT 模式响应成功>

static           uint8_t    bluetooth_ch9141_mac_address[17];              //本机mac地址
...
void bluetooth_ch9141_uart_callback_ch1 (void)
{
    ...
    if(bluetooth_ch9141_at_cmd_state == 1)                                              // 如果当前是 AT 配置模式下
    {
        fifo_read_buffer(
            &bluetooth_ch9141_fifo,                                                    // 从对应 FIFO
            (uint8_t *)&bluetooth_ch9141_at_reply[bluetooth_ch9141_at_reply_index++],   // 读取到 AT 响应缓冲区 自增响应缓冲区索引
            (uint32_t *)&bluetooth_ch9141_at_cmd_state, FIFO_READ_AND_CLEAN);           // 默认参数

        if( bluetooth_ch9141_at_reply_index >= 4 &&                                     // 获取到一段数据 判断是否是 AT 指令的成功响应
            !strncmp("OK\r\n", (const char *)&bluetooth_ch9141_at_reply[bluetooth_ch9141_at_reply_index-4], 4))
        {
            bluetooth_ch9141_at_cmd_state = 2;                                          // 切换到 AT 指令响应成功状态
        }
    }
}
...
static uint8_t bluetooth_ch9141_send_at_command (const uint8_t *str)
{
   ...
   bluetooth_ch9141_at_reply_index = 0;        // 接收数量清零
   uart_write_string(BLUETOOTH_CH9141_INDEX, (const uint8_t *)str);
   uart_write_string(BLUETOOTH_CH9141_INDEX, (const uint8_t *)"\r\n");

   // 等待收到应答信号
   while(bluetooth_ch9141_at_cmd_state != 2)
   {
       system_delay_ms(1);
       if(timeout++ > BLUETOOTH_CH9141_TIMEOUT_COUNT)                   // 超时
             return 1;
   }
   bluetooth_ch9141_at_cmd_state = 1;
   return 0;
}
...
static uint8_t bluetooth_ch9141_send_at_command_parameter (const uint8_t *cmd, const uint8_t *data)
{
   uint16_t timeout = 0;
   bluetooth_ch9141_at_reply_index = 0;        //接收数量清零
   uart_write_string(BLUETOOTH_CH9141_INDEX, (const uint8_t *)"AT+");
   uart_write_string(BLUETOOTH_CH9141_INDEX, (const uint8_t *)cmd);
   uart_write_string(BLUETOOTH_CH9141_INDEX, (const uint8_t *)"=");
   uart_write_string(BLUETOOTH_CH9141_INDEX, (const uint8_t *)data);
   uart_write_string(BLUETOOTH_CH9141_INDEX, (const uint8_t *)"\r\n");
...
static uint8_t bluetooth_ch9141_enter_at_mode (void)
{
   ...
   bluetooth_ch9141_at_cmd_state = 1;           // 进入AT模式
   bluetooth_ch9141_at_reply_index = 0;         // 接收数量清零
...
static uint8_t bluetooth_ch9141_exit_at_mode (void)
{
   if(bluetooth_ch9141_send_at_command((const uint8_t *)"AT+EXIT"))
       return 1;
   bluetooth_ch9141_at_cmd_state = 0;            //进入透传模式
   return 0;
}
...
static uint8_t bluetooth_ch9141_reset (void)
{
   ...
   if(bluetooth_ch9141_send_at_command((const uint8_t *)"AT+RESET"))
       return 1;
   while(bluetooth_ch9141_at_reply_index < 7)
   {
       system_delay_ms(1);
       if(timeout++ > BLUETOOTH_CH9141_TIMEOUT_COUNT)                            // 超时
             return 1;
   }
   bluetooth_ch9141_at_reply_index = 0;
   bluetooth_ch9141_at_cmd_state = 1;
   return 0;
}
...
static uint8_t bluetooth_ch9141_get_mac_address (void)
{
   ...
   memcpy(bluetooth_ch9141_mac_address, (uint8_t *)bluetooth_ch9141_at_reply, 17);
   return 0;
}
...
static uint8_t bluetooth_ch9141_set_mode (void)
{
   ...
   mode_data = (uint8_t)BLUETOOTH_CH9141_MODE + 0x30;
...
static bluetooth_ch9141_state_enum bluetooth_ch9141_get_status (bluetooth_ch9141_mode_enum mode)
{
   ...
   ch9141_status = (bluetooth_ch9141_at_reply[0] - '0') * 10 + (bluetooth_ch9141_at_reply[1] - '0');
...
static uint8_t bluetooth_ch9141_wait_connect_success (void)
{
   ...
   {
       system_delay_ms(1);
       ret = bluetooth_ch9141_get_status(BLUETOOTH_CH9141_MODE);
       if(timeout++ >= 9)                            // 超时
             return 1;
   }
   return 0;
...
{
   if(bluetooth_ch9141_send_at_command_parameter((const uint8_t *)"NAME", (const uint8_t *)BLUETOOTH_CH9141_SELF_NAME))
       return 1;
   if(bluetooth_ch9141_send_at_command_parameter((const uint8_t *)"PNAME", (const uint8_t *)BLUETOOTH_CH9141_SELF_NAME))
       return 1;
   return 0;
}
...
static uint8_t bluetooth_ch9141_set_password (uint8_t enable)
{
   ...
   else
   {
       ...
       if(bluetooth_ch9141_send_at_command_parameter((const uint8_t *)"PASEN", (const uint8_t *)"ON"))
           return 1;
       if(bluetooth_ch9141_send_at_command_parameter((const uint8_t *)"PASS", (const uint8_t *)BLUETOOTH_CH9141_SELF_PASSWORD))
           return 1;
   }
   return 0;
}
...
{
   return bluetooth_ch9141_send_at_command_parameter((const uint8_t *)"CONADD", (const uint8_t *)BLUETOOTH_CH9141_TAGET_INFO);
}
...
static uint8_t bluetooth_ch9141_get_connect_mac (uint8_t *mac_buffer)
{
   ...
   // mac地址为小段格式 mac_address[0] 保存的是 mac 地址最低位
    memcpy(mac_buffer, (uint8_t *)bluetooth_ch9141_at_reply, 17);
    return 0;
}
...
//-------------------------------------------------------------------------------------------------------------------
uint8_t bluetooth_ch9141_connect (void)
{
   return bluetooth_ch9141_send_at_command_parameter((const uint8_t *)"CONN", (const uint8_t *)BLUETOOTH_CH9141_TAGET_INFO);
}
...
//              需要特别注意bluetooth_ch9141_enter_at_mode函数内部有500ms的延时
//-------------------------------------------------------------------------------------------------------------------
int16_t bluetooth_ch9141_get_rssi (void)
{
   uint8_t i;
   size_t length;
   int16_t  rssi;
   ...
   if(bluetooth_ch9141_send_at_command_parameter((const uint8_t *)"RSSI", (const uint8_t *)"ON,0"))
       return 200;
   length = strlen((const char *)bluetooth_ch9141_at_reply);
   length -= 12;//计算RSSI 有多少位

   rssi = 0;
   for(i=0; i<length; i++)
   {
       rssi = rssi*10 + (bluetooth_ch9141_at_reply[0] - '0');
   }
...
uint8_t bluetooth_ch9141_init_ch1 (void)
{
   ...
   //初始化流控引脚
    ...
    bluetooth_ch9141_at_reply_index = 0;            // at模式时用于指示数据接收的数量
    bluetooth_ch9141_at_cmd_state = 0;              // OK应答命令接收成功的标志位
    memset((void *)bluetooth_ch9141_at_reply,0,30); // 接收at命令的缓存
    ...
    if(BLUETOOTH_CH9141_MASTER == BLUETOOTH_CH9141_MODE)
    {
        ...
        if(bluetooth_ch9141_connect())          //立即连接设置的从机地址
        {
