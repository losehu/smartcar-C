...
{
    //BIT_IO
//    gpio_init(BIT_WAY,     GPI, GPIO_LOW, GPIO_INT_CONFIG);
//       gpio_init(BIT_1,   GPI, GPIO_LOW, GPIO_INT_CONFIG);
//       gpio_init(BIT_2,   GPI, GPIO_LOW, GPIO_INT_CONFIG);
//       gpio_init(BIT_3,  GPI, GPIO_LOW, GPIO_INT_CONFIG);
//       gpio_init(BIT_4, GPI, GPIO_LOW, GPIO_INT_CONFIG);
//       gpio_init(BIT_5,   GPI, GPIO_LOW, GPIO_INT_CONFIG);
//       gpio_init(BIT_6,   GPI, GPIO_LOW, GPIO_INT_CONFIG);
...
//#define STC_SEC_FLAG E15// E14//E15 A3
    ...
    //D8 D9 TX RX
//    gpio_init(STC_RECV_FLAG,   GPI, GPIO_HIGH, GPIO_INT_CONFIG);
//    gpio_init(STC_FIR_FLAG,   GPI, GPIO_HIGH, GPIO_INT_CONFIG);
//    gpio_init(STC_SEC_FLAG,   GPI, GPIO_HIGH, GPIO_INT_CONFIG);
...
{
    //获取并清空计数值
    Encoder_L_Cnt = encoder_get_count(ENCODER_L_TIM)*10;
    Encoder_R_Cnt = encoder_get_count(ENCODER_R_TIM)*10;
    encoder_clean_count(ENCODER_L_TIM);
    encoder_clean_count(ENCODER_R_TIM);
    if(gpio_get_level(C6))
        Encoder_L_Cnt = -Encoder_L_Cnt;
    if(!gpio_get_level(A0))
        Encoder_R_Cnt = -Encoder_R_Cnt;
    //计算速度
...
//    CarInfo.Protect_flag = 'T';
    ...
    while(KeyLast != onepress)
    {
        ...
        ServoAdd = ServoAdd0;
        ServoWrite();
        ips114_show_int16(0, 1,(int16) gpio_get_level(STC_RECV_FLAG));
        ips114_show_int16(0, 2,(int16) gpio_get_level(STC_FIR_FLAG));
        ips114_show_int16(0, 3,(int16) gpio_get_level(STC_SEC_FLAG));
        my_delay(10);
    }
//    CarInfo.Protect_flag = 'F';
}
