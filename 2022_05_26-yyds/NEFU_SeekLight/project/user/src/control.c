...
int16 ExSpeed = 370; //320 NormalSpeed
int16 ExSpeed_SUM = 370; //250
int16 ExSpeed_SUM_tmp = 370 ;
int16 circle_enter_speed=350;///RoadWide0
int16 circle_speed = 370; //240
int16 fork_speed = 330; //350
uint16_t Ackerman_kl = 110;    //左右差速系数170
uint16_t Ackerman_kr = 110;    //120
int16 ExSpeed_L = 0;
int16 ExSpeed_R = 0;
int16 L_SingleControlOut = 0, R_SingleControlOut = 0;
int8 InControlPeriod_Flag = 'F';
int out_cnt = 0;
float diff_speed=3000; //3700diff_speed
/**
 *@Name          :Timer6_IT
...
//    Data_Filter();//均值滤波
//      ICM_I_Int();    //角度积分
//      IMUupdate(ICM_Treated.gyro.y * 0.01745, ICM_Treated.gyro.z * 0.01745, ICM_Treated.gyro.x * 0.01745, ICM_Treated.accdata.y, ICM_Treated.accdata.z, ICM_Treated.accdata.x);   //更新角度
...
//   ips114_show_int32(50, 5, right_right_cnt,4);
      //ips114_show_int16(0, 0,   (int16) VehiclesDis);
...
void DiffSpeedControl(void) {
    ...
    static last_Ackerman_kr = 150;
     speed_tmp=(float)ExSpeed_SUM;
//    if(   VehiclesDis<1200-200)speed_tmp=(float)speed_tmp-50.0*1.0*(float)VehiclesDis/1200;
////    else if(VehiclesDis>1200+100)speed_tmp=(float)speed_tmp+1.0*(float)(1.0*VehiclesDis-1200)/100*50;
//     if(   VehiclesDis<50)speed_tmp=0,stop_flag=1,ExSpeed_SUM=0;
//     if(speed_tmp>400)speed_tmp=400;
    ExSpeed = (int16)speed_tmp;

   // ips114_show_int16(1, 1,   (int16) speed_tmp);
...
void speed_IO_write(float speed) {
    //A2 +-
    //C0
    //C1
    //B0
...
//    int speed_bit = speed / 10;
//    if (speed_bit > 0) gpio_set_level(BIT_WAY, 1); //RTS2 RED
//    else  gpio_set_level(BIT_WAY, 0); //RTS2 RED
//    if (speed_bit >= 32) {
//        gpio_set_level(BIT_1, 1);
//        speed_bit -= 32;
//    } else  gpio_set_level(BIT_1, 0);
//    if (speed_bit >= 16) {
//        gpio_set_level(BIT_2, 1);
//        speed_bit -= 16;
//    } else  gpio_set_level(BIT_2, 0);
//    if (speed_bit >= 8) {
//        gpio_set_level(BIT_3, 1);
//        speed_bit -= 8;
//    } else  gpio_set_level(BIT_3, 0);
//    if (speed_bit >= 4) {
//        gpio_set_level(BIT_4, 1);
//        speed_bit -= 4;
//    } else gpio_set_level(BIT_4, 0);
//    if (speed_bit >= 2) {
//        gpio_set_level(BIT_5, 1);
//        speed_bit -= 2;
//    } else   gpio_set_level(BIT_5, 0);
//    if (speed_bit >= 1) {
//        gpio_set_level(BIT_6, 1);
//        speed_bit -= 1;
//    } else  gpio_set_level(BIT_6, 0);
}
...
//        if (Wheel_R.SpeedNow > ExSpeed_R + 50)
//            SpeedOut_R = -999;
//        if (Wheel_L.SpeedNow > ExSpeed_L + 50)
//            SpeedOut_L = -999;
