...
int16 ExSpeed = 260; //320 NormalSpeed
int16 ExSpeed_SUM = 260; //250
int16 ExSpeed_SUM_tmp=260;
int16 circle_speed = 270 ; //240
int16 fork_speed=250;
uint16_t Ackerman_kl = 220;    //左右差速系数
uint16_t Ackerman_kr = 220;
int16 ExSpeed_L = 0;
int16 ExSpeed_R = 0;
...
void AllImageDeal(void) {
    //camera_send_image(UART_6,CamImage,120*188);
    GetSimBinImage();
   search_line();
...
//    MotorWrite();
    ...
    if (CarInfo.ClosedLoopMode != NoLoop) {
        ...
        if(stop_flag==1)
          {
              if(Wheel_L.SpeedNow<=0||Wheel_R.SpeedNow<=0)
              {
                  SpeedOut_L=0,SpeedOut_R=0;
              }
          }
        pwm_l = SpeedOut_L;
        pwm_r = SpeedOut_R;
